...
func _ready() -> void:
	print("\n==============================================")
	print("   GAME GENIE SKELETAL ENGINE - UNIT TESTS    ")
	print("==============================================")
	...
	torso.rotation_degrees = 45.0
	assert(not is_zero_approx(arm.global_position.x), "Forward kinematics failed")
	print("  [PASS] 1. Forward Kinematics & Socket Hierarchy")
	...
	for px in raw:
		...
		if alpha > 0:
			assert(px == red_int or px == blue_int, "RotSprite invented unexpected colors!")
	print("  [PASS] 2. RotSprite 8x Scale2x + Rotation + Mode Filter + Palette Guard")

	# 3. Test Backfill Ring Dilation
	var occluder := Image.create_empty(16, 16, false, Image.FORMAT_RGBA8)
	occluder.set_pixel(7, 8, Color.WHITE)
	occluder.set_pixel(10, 8, Color.WHITE)
	var dilated := Backfill.dilate_occluded_region(test_img, occluder, 1)
	assert(dilated != null and not dilated.is_empty(), "Backfill dilation failed")
	print("  [PASS] 3. Backfill Ring Dilation Subsystem")

	# 4. Test Snapshot Baker
	...
	print("  [PASS] 4. Snapshot Compositor")

	# 5. Test JSON Recipe Serialization ("Art as Code")
	...
	print("  [PASS] 5. JSON Recipe Serialization & Deserialization ('Art as Code')")

	print("\n>>> ALL 5 GAME GENIE CORE ENGINE TESTS PASSED! <<<\n")
	get_tree().quit(0)
